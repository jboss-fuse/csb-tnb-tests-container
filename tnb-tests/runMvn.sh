...
cp -Rf /artifacts-tests/* /deployments/.m2/repository/
...
if [ "x$TEST_EXPR" == "x" ]; then
    mvn clean verify -ntp -fn -s $MVN_SETTINGS_PATH -P$MVN_PROFILES -Dci $MVN_ARGS -Dopenshift.namespace=$OPENSHIFT_NAMESPACE --projects org.jboss.fuse.tnb.tests.springboot:examples --also-make --also-make-dependents
else
    mvn clean verify -ntp -fn -s $MVN_SETTINGS_PATH -P$MVN_PROFILES -Dci $MVN_ARGS -Dit.test=$TEST_EXPR -Dopenshift.namespace=$OPENSHIFT_NAMESPACE --projects org.jboss.fuse.tnb.tests.springboot:examples --also-make --also-make-dependents
fi
